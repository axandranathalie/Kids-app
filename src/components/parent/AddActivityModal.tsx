...
function validate(d: {
  ...
  // "valfritt" is a valid choice, so we only guard against empty strings.
...
export function AddActivityModal({ open, onClose, onSubmit }: Props) {
  const [title, setTitle] = useState("");
  const [description, setDescription] = useState("");
  const [steps, setSteps] = useState<string[]>([""]);
  const [ageGroups, setAgeGroups] = useState<AgeGroup[]>([]);
  const [where, setWhere] = useState<WhereValue>("valfritt");
  const [when, setWhen] = useState<WhenValue>("valfritt");
...
const imagePreviewSrc = useMemo(() => {
  ...
  const { errors } = useMemo(
    () =>
      validate({
        ...
        where,
        when,
        hasImage: Boolean(imagePreviewSrc),
      }),
    [title, description, steps, ageGroups, where, when, imagePreviewSrc]
  );
 ...
 // Revoke the preview URL when it changes/unmounts.
useEffect(() => {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Lägg till ny aktivitet"
    >
      <div className="w-full max-w-lg rounded-2xl bg-white p-5 shadow-xl">
        ...
        <div className="mt-4 space-y-4">
          ...
          {/* Age */}
          <div>
            <label className="text-sm font-semibold text-gray-800">Ålder</label>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              <label className="text-sm font-semibold text-gray-800">Var</label>
              <select
                value={where}
            ...
            <div>
              <label className="text-sm font-semibold text-gray-800">När</label>
              <select
                value={when}
